...
DFS_CODE = """def dfs_traversal(graph, start):
    visited = set()
    stack = [start]
    while stack:
        curr = stack.pop()
...
def gen_dfs_traversal(graph, start):
    visited = set(); stack = [start]
    yield 4, {"stack": list(stack), "visited": list(visited)}
    while stack:
        curr = stack.pop()
        ...
        if curr not in visited:
            ...
            for neighbor in reversed(graph[curr]):
                if neighbor not in visited:
                    stack.append(neighbor)
                    yield 12, {"curr": curr, "neighbor": neighbor, "stack": list(stack)}

class DFSVisualizer(BaseVisualizer):
    def __init__(self):
        super().__init__("깊이 우선 탐색 시각화")
        self.nodes = {1:(150,150), 2:(300,50), 3:(450,150), 4:(150,300), 5:(300,300), 6:(450,300)}
        self.edges = [(1,2), (1,3), (2,4), (2,5), (3,6), (4,5)]
    ...
    def draw_stack(self, stack):
        """스택의 물리적 표현을 캔버스 하단에 그립니다."""
        self.canvas.delete("stack_ui")
        start_x = 50
        start_y = 520
        height = 30
        width = 100
        self.canvas.create_text(start_x, start_y+10, text="STACK (LIFO)", font=('Consolas', 11, 'bold'), anchor=tk.W, tags="stack_ui")
        # 스택 외곽선 (바닥이 막힌 상자)
    ...
    def get_generator(self): return gen_dfs_traversal(self.graph, 1)
    ...
    def on_step(self, ln, sd):
        ...
        if neighbor:
            ...
            self.canvas.create_text(nx, ny-35, text="PUSH", fill="#8e44ad", font=('Arial', 9, 'bold'), tags="highlight")

    def on_reset(self): 
        self.set_code(DFS_CODE)
        self.draw_graph()
        self.draw_stack([])

if __name__ == "__main__": DFSVisualizer().mainloop()
